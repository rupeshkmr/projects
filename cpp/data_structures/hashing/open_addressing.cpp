...
// i = probe interval
int linearProbe(int hash_val,int i,string s){
  if(table[hash_val] == "" || table[hash_val] == s){
    ...
    hash_val = (hash_val+i)%m;
    probeLimit++;
    if(probeLimit == 21)
      return -1;
    //finish up
    return linearProbe(hash_val,i,s);
  }
}

//Searching for hash value stored in the lists
void search(string s){
  int slot = linearProbe(hashmod20(s),1,s);
...
void hash_(vector <string> s){
  for(int i=0;i<s.size();i++){
    ...
    int slot = linearProbe(hash_val,1,s[i]);
    table[slot] = s[i];
