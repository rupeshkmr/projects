...
//Hash function#2
//compute ascii sum of each alphabet of the string and take remainder modulo 35
int hashmod35(string s1){
  int asciisum=0;
  for(int i=0;i<s1.length();i++){
    asciisum += s1[i];
  }
...
// i = probe interval
int Probe(int hash_val,int i,string s){
  if(table[hash_val] == "" || table[hash_val] == s){
    ...
    hash_val = (hash_val+i*hashmod35(s))%m;
    probeLimit++;
    if(probeLimit == 21)
      return -1;
    //finish up
    return Probe(hash_val,i+1,s);
  }
}

//Searching for hash value stored in the lists
void search(string s){
  int slot = Probe(hashmod20(s),1,s);
...
int main(){
  ...
  search("fine");
  for(int i=0;i<s.size();i++){
    cout<<" Hash : "<<s[i]<<" = "<<hashmod35(s[i])<<endl;
  }
